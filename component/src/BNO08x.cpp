...
bool BNO08x::initialize()
{
    ...
    ESP_LOGI(TAG, "[INIT] Starting init_config_args()");
    if (init_config_args() != ESP_OK) {
        ESP_LOGE(TAG, "[INIT] init_config_args() failed");
        return false;
    }
    ESP_LOGI(TAG, "[INIT] init_config_args() success");

    // initialize GPIO
    ESP_LOGI(TAG, "[INIT] Starting init_gpio()");
    if (init_gpio() != ESP_OK) {
        ESP_LOGE(TAG, "[INIT] init_gpio() failed");
        return false;
    }
    ESP_LOGI(TAG, "[INIT] init_gpio() success");

    // initialize HINT ISR
    ESP_LOGI(TAG, "[INIT] Starting init_hint_isr()");
    if (init_hint_isr() != ESP_OK) {
        ESP_LOGE(TAG, "[INIT] init_hint_isr() failed");
        return false;
    }
    ESP_LOGI(TAG, "[INIT] init_hint_isr() success");

    // initialize SPI
    ESP_LOGI(TAG, "[INIT] Starting init_spi()");
    if (init_spi() != ESP_OK) {
        ESP_LOGE(TAG, "[INIT] init_spi() failed");
        return false;
    }
    ESP_LOGI(TAG, "[INIT] init_spi() success");

    // initialize SH2 HAL
    ESP_LOGI(TAG, "[INIT] Starting init_sh2_HAL()");
    if (init_sh2_HAL() != ESP_OK) {
        ESP_LOGE(TAG, "[INIT] init_sh2_HAL() failed");
        return false;
    }
    ESP_LOGI(TAG, "[INIT] init_sh2_HAL() success");

    // initialize tasks
    ESP_LOGI(TAG, "[INIT] Starting init_tasks()");
    if (init_tasks() != ESP_OK) {
        ESP_LOGE(TAG, "[INIT] init_tasks() failed");
        return false;
    }
    ESP_LOGI(TAG, "[INIT] init_tasks() success");
...
esp_err_t BNO08x::init_gpio_inputs()
{
    ...
    inputs_config.pull_up_en = GPIO_PULLUP_ENABLE;   // Enable pull-up for open-drain HINT
...
esp_err_t BNO08x::init_sh2_HAL()
{
    ...
    ESP_LOGI(TAG, "[SH2] Setting HAL IMU");
    BNO08xSH2HAL::set_hal_imu(this);

    // register sh2 HAL callbacks
    ESP_LOGI(TAG, "[SH2] Registering SH2 HAL callbacks");
    ...
    // reset BNO08x
    ESP_LOGI(TAG, "[SH2] Toggling BNO08x reset pin");
    toggle_reset();
    ESP_LOGI(TAG, "[SH2] Reset complete");

    ESP_LOGI(TAG, "[SH2] Calling sh2_open()");
    int hint_level_before = gpio_get_level(imu_config.io_int);
    ESP_LOGI(TAG, "[SH2] HINT pin level before sh2_open: %d", hint_level_before);
    
    if (sh2_open(&sh2_HAL, BNO08xSH2HAL::hal_cb, NULL) != SH2_OK)
    {
        ESP_LOGE(TAG, "[SH2] sh2_open() failed");
        return ESP_FAIL;
    }
    
    int hint_level_after = gpio_get_level(imu_config.io_int);
    ESP_LOGI(TAG, "[SH2] HINT pin level after sh2_open: %d", hint_level_after);
    ESP_LOGI(TAG, "[SH2] sh2_open() success");

    init_status.sh2_HAL = true;

    memset(&product_IDs, 0, sizeof(sh2_ProductIds_t));

    ESP_LOGI(TAG, "[SH2] Calling sh2_getProdIds()");
    int hint_level_before_prodids = gpio_get_level(imu_config.io_int);
    ESP_LOGI(TAG, "[SH2] HINT pin level before getProdIds: %d", hint_level_before_prodids);
    
    TickType_t start_time = xTaskGetTickCount();
    int prod_id_result = sh2_getProdIds(&product_IDs);
    TickType_t elapsed_ticks = xTaskGetTickCount() - start_time;
    uint32_t elapsed_ms = elapsed_ticks * portTICK_PERIOD_MS;
    
    int hint_level_after_prodids = gpio_get_level(imu_config.io_int);
    ESP_LOGI(TAG, "[SH2] getProdIds elapsed: %ldms, HINT level after: %d, result: %d", 
             elapsed_ms, hint_level_after_prodids, prod_id_result);
    
    if (prod_id_result != SH2_OK)
    {
        ESP_LOGE(TAG, "[SH2] sh2_getProdIds() TIMEOUT or FAILED!");
        ESP_LOGE(TAG, "[SH2] HINT pin stuck at: %d, expected to go LOW on data", hint_level_after_prodids);
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "[SH2] sh2_getProdIds() success");

    ESP_LOGI(TAG, "[SH2] Calling sh2_setSensorCallback()");
    if (sh2_setSensorCallback(BNO08xSH2HAL::sensor_event_cb, NULL) != SH2_OK) {
        ESP_LOGE(TAG, "[SH2] sh2_setSensorCallback() failed");
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "[SH2] sh2_setSensorCallback() success");
